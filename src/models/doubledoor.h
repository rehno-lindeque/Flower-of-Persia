#ifndef __DOUBLEDOOR_H__
#define __DOUBLEDOOR_H__

class DoubleDoor : public Model
{
public:  
  ...
  void drawDoubleDoor(float* arches)
  {
    glBegin(GL_QUADS);
      for(int c = 0; c < 1; c++)
      {
        // draw collumns
        extrudeYQuad(colRadius, startHeight, collumnHeight, -gap - colRadius, arches[2]);
        extrudeYQuad(colRadius, startHeight, collumnHeight,  gap + colRadius, arches[2]);

        // draw upper arc
        drawFrontArc(arches);
        drawBackArc(arches);
        drawInsideArc(arches);
        drawOutsideArc(arches);
      }        
    glEnd();
  }
  ...
  {    
    ...
    float arches[][3] = 
    { {0.0f, 0.0f, -36.0+colRadius}, {0.0f, 0.0f, 0.0} };

    displayList = glGenLists(1);
    glNewList(displayList, GL_COMPILE);    
      glPushMatrix();
        drawDoubleDoor(arches[0]);
      glPopMatrix();
      glPushMatrix();
        glTranslatef(24.0f, 11.2f, 0.0f);
        glRotatef(90.0f, 0.0f, 1.0f, 0.0f);
        drawDoubleDoor(arches[1]);
      glPopMatrix();
      glPushMatrix();
        glTranslatef(-24.0f, 11.2f, 0.0f);
        glRotatef(90.0f, 0.0f, 1.0f, 0.0f);
    ...
    glNewList(normalList, GL_COMPILE);    
      ...
      glBegin(GL_LINES);        
        ...
        {
          // draw collumns
          extrudeYQuad_drawNormals(colRadius, startHeight, collumnHeight, -gap - colRadius, arches[c][2]);
          extrudeYQuad_drawNormals(colRadius, startHeight, collumnHeight,  gap + colRadius, arches[c][2]);
