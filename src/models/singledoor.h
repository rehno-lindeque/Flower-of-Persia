#ifndef __SINGLEDOOR_H__
#define __SINGLEDOOR_H__

class SingleDoor : public Model
{
public:  
  GLuint displayList;
  ...
  {    
    ...
    glNewList(displayList, GL_COMPILE);    
      glPushMatrix();
      glTranslatef(-22.0f, 1.4f, 26.0f);
      glRotatef(-45.0f, 0.0f, 1.0f, 0.0f);      
      glBegin(GL_QUADS);        
        {          
          // draw collumns
          extrudeYQuad(colRadius, startHeight, collumnHeight, -gap - colRadius, arches[0][2]);
          extrudeYQuad(colRadius, startHeight, collumnHeight,  gap + colRadius, arches[0][2]);

          // draw upper arc
          drawFrontArc(arches[0]);
          //drawBackArc(arches[0]);
          drawInsideArc(arches[0]);
          drawOutsideArc(arches[0]);
        }        
      glEnd();
      glRotatef(45.0f, 0.0f, 1.0f, 0.0f);      
      glTranslatef(44.0f, 0.0f, 0.0f); // note: this is relative to the first door
      glRotatef(45.0f, 0.0f, 1.0f, 0.0f);
      glBegin(GL_QUADS);        
        {          
          // draw collumns
